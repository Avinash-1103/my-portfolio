  ...
  return (
    <section id="projects" className="max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((p) => (
          <motion.a key={p.id} whileHover={{ scale: 1.03 }} className="block p-5 rounded-2xl bg-white dark:bg-gray-800 shadow hover:shadow-lg ring-1 ring-gray-100 dark:ring-gray-700" href={p.link} target="_blank" rel="noreferrer">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {p.tech.map(t => <span key={t} className="text-xs px-2 py-1 rounded-md bg-gray-100 dark:bg-gray-700">{t}</span>)}
            </div>
          </motion.a>
        ))}
      </div>
